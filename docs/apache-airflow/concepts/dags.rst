...
There are several ways to prevent a dag from running. DAGs can be paused, deactivated, or deleted.
...
via the UI and the API.  In the UI, you can see Paused DAGs in the ``Paused`` tab. DAGs that are un-paused
can be found in the ``Active`` tab.

A DAG can also be deactivated. A deactivated DAG will not run, cannot be triggered, and won't show up in the UI but its metadata 
and history persists in the database. If a DAG is reactivated all of its history and metadata will be present.
...
This all means that if you want to completely delete a DAG and its all historical metadata, you need to do
it in three steps:
